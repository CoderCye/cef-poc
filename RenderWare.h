...
using RwV2d = struct RwV2d;
using RwV3d = struct RwV3d;
using RwPlane = struct RwPlane;
using RwBBox = struct RwBBox;
using RpGeometry = struct RpGeometry;
using RpWorld = void;
using RwRasterLockFlags = enum RwRasterLockFlags;
using RpClump = struct RpClump;
using RwRaster = struct RwRaster;
using RpMaterialLighting = struct RpMaterialLighting;
using RpMaterialList = struct RpMaterialList;
using RpMaterial = struct RpMaterial;
using RpTriangle = struct RpTriangle;
using RwTextureCoordinates = struct RwTextureCoordinates;
using RwColor = struct RwColor;
using RwColorFloat = struct RwColorFloat;
using RpLightType = enum RpLightType;
using RpLightFlags = enum RpLightFlags;
using RwObjectFrame = struct RwObjectFrame;
using RpAtomic = struct RpAtomic;
using RwCamera = struct RwCamera;
using RpLight = struct RpLight;
using RwCameraType = enum RwCameraType;

using RwCameraPreCallback = RwCamera*(*)(RwCamera* camera);
using RwCameraPostCallback = RwCamera*(*)(RwCamera* camera);
using RpAtomicCallback = RpAtomic*(*)(RpAtomic* atomic);
...
using RwIOCallbackClose = int(*)(void* data);
using RwIOCallbackRead = unsigned int(*) (void* data, void* buffer, unsigned int length);
using RwIOCallbackWrite = int(*)(void* data, const void* buffer, unsigned int length);
using RwIOCallbackOther = int(*)(void* data, unsigned int offset);
